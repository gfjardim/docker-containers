...
if [[ ! -L "/usr/local/crashplan/bin" ]]; then
  if [ ! -f "/config/bin/run.conf" ]; then
    cp -f /usr/local/crashplan/bin/run.conf /config/bin/run.conf
  fi
...
# noVNC
sed -i -e "s#WEB_PORT#${TCP_PORT_4280}#g" /etc/services.d/novnc/run
sed -i -e "s#VNC_PORT#${TCP_PORT_4239}#g" /etc/services.d/novnc/run

# TigerVNC
sed -i -e "s#VNC_PORT#${TCP_PORT_4239}#g" /etc/services.d/tigervnc/run

if [[ -n $VNC_PASSWD ]]; then
  sed -i -e "s#SECURITY#-SecurityTypes TLSVnc,VncAuth -PasswordFile /nobody/.vnc_passwd#g" /etc/services.d/tigervnc/run
  /opt/vncpasswd/vncpasswd.py -f /config/.vnc_passwd -e "${VNC_PASSWD}"
else
  sed -i -e "s#SECURITY#-SecurityTypes None#g" /etc/services.d/tigervnc/run
fi
...
sed -i -e "s#SERVICE_PORT#${TCP_PORT_4243}#g" /etc/init.d/crashplan
